...
from notifier import google_sheets
...
def run():
    ...
    try:
        google_sheets.append_jobs(new_jobs)
        # Only mark jobs as seen after Google Sheets accepted them. This makes a
        # failed Sheet update retry on the next scheduled run instead of losing jobs.
        for job in new_jobs:
            db.save_job(job)
        print(f"[{datetime.now()}] Run complete. New jobs saved to Google Sheets.")
    except Exception as e:
        print(f"[{datetime.now()}] ERROR: Google Sheets update failed — {e}")
        print("  Scraping completed successfully; only the Google Sheets update failed.")
        raise
