...
# Kartik has ~2.5 years' experience, so roles asking for up to three years remain eligible.
...
# Catch trailing level suffix: "Engineer - 3" — level 3+ is outside this target.
_TRAILING_LEVEL_RE = re.compile(r'[-–]\s*([3-9])\s*$')


def _is_senior_by_experience(title: str) -> bool:
    m = _EXP_RE.search(title)
    if m and int(m.group(1)) > MAX_EXPERIENCE_YEARS:
        return True
    m2 = _PAREN_RANGE_RE.search(title)
    if m2 and int(m2.group(1)) > MAX_EXPERIENCE_YEARS:
        return True
    # Trailing level suffix: "Engineer - 3" and above generally exceeds this level.
